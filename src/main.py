...
import argparse
import logging
from commands.cli_commands import handle_config_cli_command, handle_activity_cli_command, handle_skill_cli_command, handle_plan_cli_command, handle_practice_cli_command
...
if __name__ == '__main__':
    ...
    parser_config = subparsers.add_parser(
        "config", help="sets a new configuration"
    )
    parser_activity = subparsers.add_parser(
        "activity", help="manage activities"
    )
    parser_skill = subparsers.add_parser(
        "skill", help="manage skills"
    )
    parser_plan = subparsers.add_parser(
        "plan", help="manage plans"
    )
    parser_practice = subparsers.add_parser(
        "practice", help="manage practices"
    )

    # config
    parser_config.add_argument(
        "--directory", required=True, help="the name of the directory activities are stored (will be converted to absolute)"
    )
    ...
    parser_skill_ls = subparsers_skill.add_parser(
        "ls", help="list all activities"
    )
    ...
    parser_plan_create = subparsers_plan.add_parser(
        "create", help="create a new plan"
    )
    parser_plan_ls = subparsers_plan.add_parser(
        "ls", help="list all activities"
    )
    ...
    parser_practice_ls = subparsers_practice.add_parser(
        "ls", help="list all practices"
    )
    ...
    parser_practice_create.add_argument(
        "--name", required=True, help="the name of the new practice"
    )
